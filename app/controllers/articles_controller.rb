class ArticlesController < ApplicationController
  ...
  def index
    ...
    unless comment_count.blank?
      ...
      puts 'has_no_comment_count'
      @articles = Article.search(params[:q]).result.includes(:user).page(params[:page])
    end
  end
  ...
  private
    ...
    def article_params
      params.require(:article).permit(:user_id, :title, :content, :category_id, :publish_at,
        :comments_attributes => [:id, :name, :content, :article_id, :_destroy])
    end
end
